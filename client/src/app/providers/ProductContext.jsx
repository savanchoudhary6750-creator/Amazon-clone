...
export const ProductProvider = ({ children }) => {
  ...
  const fetchProducts = useCallback(async (filterParams = {}) => {
    ...
    try {
      const response = await productService.getAll(filterParams);
  ...
  const searchProducts = useCallback(async (query, filterParams = {}) => {
    ...
    try {
      const response = await productService.search(query, filterParams);
      setProducts(response.data.products);
      setPagination(response.data.pagination);
  ...
  const getProductsByCategory = useCallback(async (category, limit = 20) => {
    ...
    try {
      const response = await productService.getByCategory(category, limit);
      setProducts(response.data.products);
      return response.data.products;
    } catch (err) {
      setError(err.data?.message || err.message);
  ...
  const addReview = useCallback(async (productId, review) => {
    ...
    try {
      const response = await productService.addReview(productId, review);
      return response.data;
    } catch (err) {
      setError(err.data?.message || err.message);
      throw err;
